...
SparseMatrix BSplineBasis1D::refineKnotsLocally(double x)
{
    ...
    if (getNumBasisFunctions() >= getNumBasisFunctionsTarget() || assertNear(knots.front(), knots.back()))
    {
    ...
    if (knotMultiplicity(x) > 0 || assertNear(*upper, x, 1e-6, 1e-6) || assertNear(*lower, x, 1e-6, 1e-6))
    {
