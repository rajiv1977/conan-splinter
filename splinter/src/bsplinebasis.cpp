    ...
    : numVariables(knotVectors.size())
{
    if (knotVectors.size() != basisDegrees.size())
        throw Exception("BSplineBasis::BSplineBasis: Incompatible sizes. Number of knot vectors is not equal to size "
                        "of degree vector.");
...
// NOTE: does not pass tests << Note unit test has failed
SparseMatrix BSplineBasis::evalBasisJacobian(DenseVector& x) const
{
    ...
    for (unsigned int i = 0; i < numVariables; ++i)
    {
        ...
        for (int k = 0; k < Ji.outerSize(); ++k)
            for (SparseVector::InnerIterator it(Ji, k); it;
                 ++it) // In new Eigen lib version SparseMatrix::InnerIterator  is inaccessible
