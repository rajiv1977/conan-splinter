...
extern "C"
{
     ...
     */
    SPLINTER_API void
        splinter_datatable_add_samples_row_major(splinter_obj_ptr datatable_ptr, double* x, int n_samples, int x_dim);
     ...
     */
    SPLINTER_API void
        splinter_datatable_add_samples_col_major(splinter_obj_ptr datatable_ptr, double* x, int n_samples, int x_dim);
     ...
     */
    SPLINTER_API void
        splinter_bspline_builder_set_degree(splinter_obj_ptr bspline_builder_ptr, unsigned int* degrees, int n);
     ...
     */
    SPLINTER_API void splinter_bspline_builder_set_num_basis_functions(splinter_obj_ptr bspline_builder_ptr,
    ...
    /**
     * Evaluate a BSpline in one or more points. Can "batch evaluate" several points by storing the points consecutively
     * in x in row major order. If function is a two-dimensional function you can evaluate the function in x0 = [0, 1]
     * and x1 = [2, 3] in one function call by having x point to the start of an array of four doubles: 0 1 2 3, and
     * x_len = 4. This function will then return an array of 2 doubles, the first being the result of evaluating the
     * function in [0, 1], and the second being the result of evaluating the function in [2, 3].
     *
    ...
    /**
     * Insert knots at tau of multiplicity 'multiplicity' to knot vector in variable 'dim'. The B-spline is
     * geometrically unaltered by the knot insertion.
     *
     * @param bspline_ptr Pointer to the BSpline to evaluate.
     ...
     */
    SPLINTER_API void splinter_bspline_insert_knots(splinter_obj_ptr bspline_ptr,
                                                    double           tau,
                                                    unsigned int     dim,
